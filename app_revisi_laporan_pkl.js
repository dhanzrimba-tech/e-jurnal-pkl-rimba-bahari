...
async function renderStudentGroupReport() {
  ...
  $('#content').innerHTML = `<div class="page-intro final-report-intro"><div><span class="section-kicker">LAPORAN AKHIR PKL KELOMPOK</span><h3>${esc(context.internship_place || 'Lokasi PKL')}</h3><p>Anggota kelompok dibentuk otomatis dari siswa yang memiliki lokasi praktik yang sama. Isi naratif laporan dipakai bersama seluruh anggota.</p></div><div class="report-action-buttons"><button class="btn secondary" id="backIndividualReport">Laporan Individu</button><button class="btn secondary" id="previewGroupReport">Pratinjau</button><button class="btn secondary" id="downloadWordGroupReport">Unduh Word (.doc)</button><button class="btn primary" id="printGroupReport">Cetak / Simpan PDF</button></div></div>
    ...
    <div class="info-strip final-report-auto-info"><strong>Data otomatis kelompok:</strong> daftar anggota dan BAB III Pembahasan menggunakan maksimal <b>2 jurnal paling relevan</b> dari jurnal <b>Disetujui</b> pada lokasi ini. Jika materi sama atau sangat mirip, sistem memilih jurnal dengan isi dan dokumentasi paling lengkap. Jurnal lainnya tetap tersedia pada lampiran. Daftar Gambar dan Daftar Pustaka dibuat otomatis. Catatan pembimbing tidak ditampilkan di BAB III.</div><div class="info-strip"><strong>Pemeriksaan sumber:</strong> sitasi ilmiah dipilih otomatis berdasarkan isi jurnal anggota. Guru Pembimbing perlu memeriksa kesesuaiannya sebelum menyetujui laporan kelompok.</div>
...
function reportFigureListHtml(figures) {
  ...
  return `<table class="figure-list">${figures.map((figure) => `<tr><td><b>Gambar ${esc(figure.number)}</b></td><td class="leader-cell"><span>${esc(figure.caption)}</span><span class="leader"></span><span class="page-placeholder">-</span></td></tr>`).join('')}</table>`;
}

function reportTocRowsHtml({ group = false } = {}) {
  const rows = [
    ['LEMBAR PENGESAHAN', 'i'],
    ['KATA PENGANTAR', 'ii'],
    ['DAFTAR ISI', 'iii'],
    ['DAFTAR GAMBAR', 'iv'],
    ['BAB I PENDAHULUAN', '1'],
    ['BAB II PROFIL TEMPAT PRAKTIK KERJA LAPANGAN', '2'],
    ['BAB III PEMBAHASAN PRAKTIK KERJA LAPANGAN', '3'],
    ['BAB IV PENUTUP', '4'],
    ['DAFTAR PUSTAKA', '5'],
    ['LAMPIRAN', ''],
  ];
  return rows.map(([label, page]) => `<tr><td class="toc-label">${esc(label)}</td><td class="leader-cell"><span></span><span class="leader"></span><span class="toc-page">${esc(page)}</span></td></tr>`).join('');
}
...
function reportJournalQualityScore(journal) {
  const description = String(journal?.description || '').trim();
  const learning = String(journal?.learning || '').trim();
  const obstacles = String(journal?.obstacles || '').trim();
  const reflection = String(journal?.reflection || '').trim();
  const stages = Array.isArray(journal?.activity_stages) ? journal.activity_stages.filter(Boolean) : [];
  const photos = Array.isArray(journal?.photo_paths) ? journal.photo_paths.filter(Boolean) : [];
  const reference = researchReferenceForJournal(journal);

  let score = 0;
  score += Math.min(description.length, 650) / 45;
  score += Math.min(learning.length, 500) / 45;
  score += Math.min(obstacles.length, 350) / 55;
  score += Math.min(reflection.length, 350) / 55;
  score += Math.min(stages.length, 7) * 1.8;
  score += Math.min(photos.length, 3) * 8;
  score += Number(journal?.work_hours || 0) > 0 ? 2 : 0;
  score += reference?.id !== 'pkl-general' ? 6 : 0;
  return score;
}

function reportJournalTopicKey(journal) {
  const reference = researchReferenceForJournal(journal);
  if (reference && reference.id !== 'pkl-general') return `reference:${reference.id}`;

  const stopWords = new Set([
    'kegiatan', 'melaksanakan', 'pelaksanaan', 'pengenalan', 'penjelasan',
    'tentang', 'praktik', 'kerja', 'lapangan', 'hari', 'siswa', 'pkL',
    'yang', 'dan', 'dengan', 'untuk', 'pada', 'di', 'ke', 'dari', 'apa',
    'itu', 'cara', 'serta', 'materi', 'belajar', 'pembelajaran',
  ]);

  const title = String(journal?.activity_title || '')
    .toLocaleLowerCase('id-ID')
    .replace(/[^a-z0-9\s]/g, ' ');

  const words = [...new Set(title.split(/\s+/).filter((word) => word.length >= 4 && !stopWords.has(word)))];
  if (words.length) return `title:${words.slice(0, 5).sort().join('|')}`;

  return 'topic:general';
}

function reportJournalTokenSet(journal) {
  const text = [
    journal?.activity_title,
    journal?.description,
    ...(Array.isArray(journal?.activity_stages) ? journal.activity_stages : []),
...
function reportJournalSimilarity(a, b) {
  const left = reportJournalTokenSet(a);
  const right = reportJournalTokenSet(b);
...
function selectFeaturedReportJournals(journals, maxItems = 2) {
  const source = Array.isArray(journals)
    ? journals.filter((journal) => journal && journal.status === 'approved')
    : [];
  ...
  const ranked = source
    .map((journal, index) => ({
      journal,
      index,
      score: reportJournalQualityScore(journal),
      topic: reportJournalTopicKey(journal),
    }))
  ...
  for (const item of ranked) {
    ...
    const sameTopic = usedTopics.has(item.topic);
    const tooSimilar = selected.some((picked) => reportJournalSimilarity(picked, item.journal) >= 0.62);

    if (sameTopic || tooSimilar) continue;

    selected.push(item.journal);
    usedTopics.add(item.topic);
  }

  // If all journals are very similar, keep only the single best one.
  if (selected.length > 1 && reportJournalSimilarity(selected[0], selected[1]) >= 0.62) {
    selected.splice(1);
  }

  // Preserve chronological order in the final report.
  return selected.sort((a, b) => {
    const left = String(a?.journal_date || '');
    const right = String(b?.journal_date || '');
    return left.localeCompare(right);
  });
}

function reportDiscussionJournals(journals, maxItems = 2) {
  return selectFeaturedReportJournals(journals, maxItems);
}

function reportDiscussionHtml(journals, imageSources, { group = false, membersCount = 1, totalHours = 0 } = {}) {
  const selectedJournals = reportDiscussionJournals(journals, 2);

  if (!selectedJournals.length) {
    return '<h3 class="subchapter">3.1 Ringkasan Hasil Praktik Kerja Lapangan</h3><p>Belum ada jurnal berstatus Disetujui yang dapat dibahas.</p>';
  }

  const figures = reportFigureItems(selectedJournals, { group });
  ...
  const topicKey = (journal) => {
    const raw = String(journal?.activity_title || 'Materi PKL').trim();
    const reference = researchReferenceForJournal(journal);
    if (reference?.id && reference.id !== 'pkl-general') return reference.id;
    return raw.toLocaleLowerCase('id-ID').replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim() || 'materi-pkl';
  };

  const topicLabel = (journal) => {
    const raw = String(journal?.activity_title || 'Materi PKL').trim();
    const cleaned = raw.replace(/^(pengenalan|materi|kegiatan|praktik|pelaksanaan)\s+/i, '').trim();
    return cleaned || raw || 'Materi PKL';
  };

  const groups = [];
  const byKey = new Map();
  selectedJournals.forEach((journal, index) => {
    const key = topicKey(journal);
    if (!byKey.has(key)) {
      const groupItem = { key, label: topicLabel(journal), items: [] };
      byKey.set(key, groupItem);
      groups.push(groupItem);
    }
    byKey.get(key).items.push({ journal, index });
  });

  const summary = `<h3 class="subchapter">3.1 Ringkasan Hasil Praktik Kerja Lapangan</h3><p>Pembahasan BAB III diringkas dari ${selectedJournals.length} jurnal paling relevan${groupText}. Jurnal yang membahas materi sama atau sangat mirip digabungkan ke dalam satu pokok materi agar pembahasan tidak berulang. Jurnal lainnya tetap tersimpan dan dicantumkan pada lampiran.</p>`;

  const details = groups.map((topic, topicIndex) => {
    const main = topic.items[0].journal;
    const title = compactReportText(topic.label, 120);
    const subDetails = topic.items.map(({ journal, index }, subIndex) => {
      ...
      const photo = figure
        ? `<figure class="discussion-photo">${imageSources[figure.path] ? `<img src="${esc(imageSources[figure.path])}" alt="${esc(figure.caption)}">` : '<div class="photo-missing">Foto tidak dapat dimuat</div>'}<figcaption><b>Gambar ${esc(figure.number)}.</b> ${esc(figure.caption)}</figcaption></figure>`
        : '';
      return `<div class="topic-subdiscussion"><h4 class="activity-discussion-title">3.${topicIndex + 2}.${subIndex + 1} ${esc(journal.activity_title || title)}</h4><p>${firstParagraph}</p>${outcomeParts.length ? `<p>${outcomeParts.join(' ')}</p>` : ''}${literature}${photo}</div>`;
    }).join('');
    return `<div class="activity-topic"><h3 class="subchapter">3.${topicIndex + 2} ${esc(title)}</h3><p class="topic-intro">Pokok materi ini dirangkum dari ${topic.items.length} jurnal yang memiliki materi sama atau saling berkaitan. Uraian disatukan untuk menghindari pengulangan dan tetap mempertahankan pengalaman praktik siswa.</p>${subDetails}</div>`;
  }).join('');

  return `${summary}<h3 class="subchapter">3.2 Pembahasan Berdasarkan Pokok Materi</h3>${details}`;
}
...
async function printFinalPklReport(studentId, { autoPrint = false } = {}) {
  ...
  try {
    ...
    const approved = journals.filter((item) => item.status === 'approved');
    const discussionJournals = reportDiscussionJournals(approved, 2);
    const photoPaths = [...new Set(discussionJournals.map((item) => (item.photo_paths || []).filter(Boolean)[0]).filter(Boolean))];
    const signedUrls = {};
    const batchSize = 50;
    ...
    const reportFigures = reportFigureItems(discussionJournals);
    const reportReferences = referencesUsedByJournals(discussionJournals);
    ...
    const html = `<!doctype html><html lang="id"><head><meta charset="utf-8"><title>${esc(title)} - ${esc(studentName)}</title><style>
      ...
      <section class="page prelim-page"><h2 class="chapter-title">LEMBAR PENGESAHAN</h2><div class="page-number center-page-no">i</div><p>Laporan Praktik Kerja Lapangan ini telah diperiksa dan disetujui sebagai dokumentasi pelaksanaan PKL siswa.</p><table class="approval-table"><tr><th>Nama Siswa</th><td>${esc(studentName)}</td></tr><tr><th>NISN / Kelas</th><td>${esc(nis)} / ${esc(className)}</td></tr><tr><th>Tempat PKL</th><td>${esc(internshipPlace)}</td></tr><tr><th>Unit Penempatan</th><td>${esc(report.placement_unit || '-')}</td></tr><tr><th>Periode PKL</th><td>${esc(period)}</td></tr><tr><th>Status Laporan</th><td>${esc(finalReportStatusMeta(report.status)[0])}</td></tr></table><p class="right">${esc(settings.approval_location || 'Sumedang')}, ${esc(new Intl.DateTimeFormat('id-ID',{dateStyle:'long'}).format(new Date()))}</p><div class="signature-grid"><div>Pembimbing Lapangan<div class="space"></div><b>${esc(fieldSupervisorName)}</b></div><div>Guru Pembimbing<div class="space"></div><b>${esc(teacherName)}</b></div><div>Siswa<div class="space"></div><b>${esc(studentName)}</b></div><div>Kepala Sekolah<div class="space"></div><b>${esc(principalName)}</b>${settings.principal_nip ? `<br>NIP. ${esc(settings.principal_nip)}` : ''}</div></div></section>
      <section class="page prelim-page"><h2 class="chapter-title">KATA PENGANTAR</h2><div class="page-number center-page-no">ii</div><p class="pre">${textBlock(report.preface)}</p></section>
      <section class="page prelim-page"><h2 class="chapter-title">DAFTAR ISI</h2><div class="page-number center-page-no">iii</div><table class="toc">${reportTocRowsHtml()}</table><p class="report-note">Nomor halaman dapat disesuaikan setelah dokumen disimpan sebagai PDF atau dicetak.</p></section>
      <section class="page prelim-page"><h2 class="chapter-title">DAFTAR GAMBAR</h2><div class="page-number center-page-no">iv</div>${reportFigureListHtml(reportFigures)}</section>
      <section class="page chapter-page"><h2 class="chapter-title">BAB I<br>PENDAHULUAN</h2><div class="page-number center-page-no">1</div><h3 class="subchapter">1.1 Latar Belakang</h3><p class="pre">${textBlock(settings.standard_background)}</p><h3 class="subchapter">1.2 Tujuan PKL</h3><p class="pre">${textBlock(settings.standard_objectives)}</p><h3 class="subchapter">1.3 Manfaat PKL</h3><p class="pre">${textBlock(settings.standard_benefits)}</p><h3 class="subchapter">1.4 Waktu dan Tempat Pelaksanaan</h3><div class="meta-box"><p><b>Tempat PKL:</b> ${esc(internshipPlace)}</p><p><b>Unit/Bagian:</b> ${esc(report.placement_unit || '-')}</p><p><b>Periode:</b> ${esc(period)}</p></div></section>
      <section class="page chapter-page"><h2 class="chapter-title">BAB II<br>PROFIL TEMPAT PRAKTIK KERJA LAPANGAN</h2><div class="page-number center-page-no">2</div><h3 class="subchapter">2.1 Identitas dan Gambaran Umum Instansi</h3><p class="pre">${textBlock(report.institution_profile)}</p><h3 class="subchapter">2.2 Struktur Organisasi / Posisi Penempatan</h3><p class="pre">${textBlock(report.organization_structure, 'Struktur organisasi tidak dicantumkan.')}</p><h3 class="subchapter">2.3 Bidang atau Bagian Penempatan Siswa</h3><p>Siswa melaksanakan PKL pada unit/bagian <b>${esc(report.placement_unit || '-')}</b> di ${esc(internshipPlace)}.</p></section>
      <section class="page chapter-page"><h2 class="chapter-title">BAB III<br>PEMBAHASAN PRAKTIK KERJA LAPANGAN</h2><div class="page-number center-page-no">3</div>${reportDiscussionHtml(discussionJournals, signedUrls, { totalHours })}</section>
      
      
      <section class="page chapter-page"><h2 class="chapter-title">BAB IV<br>PENUTUP</h2><div class="page-number center-page-no">4</div><h3 class="subchapter">4.1 Kesimpulan</h3><p class="pre">${textBlock(report.conclusion)}</p>${suggestions}</section>
      <section class="page prelim-page"><h2 class="chapter-title">DAFTAR PUSTAKA</h2><div class="page-number right-page-no">5</div>${reportBibliographyHtml(reportReferences)}</section>
      <section class="page appendix-page"><h2 class="chapter-title">LAMPIRAN 1<br>REKAPITULASI JURNAL HARIAN</h2>${approved.length ? approved.map((item,index)=>`<div class="journal-appendix"><h4>${index+1}. ${esc(formatAttendanceDate(item.journal_date))} - ${esc(item.activity_title || 'Kegiatan PKL')}</h4><p><b>Lokasi:</b> ${esc(item.location || '-')} | <b>Jam:</b> ${esc(item.work_hours || 0)}</p><p><b>Uraian:</b> ${esc(item.description || '-')}</p><p><b>Tahapan:</b> ${esc((item.activity_stages || []).join(', ') || '-')}</p><p><b>Pengetahuan/Keterampilan:</b> ${esc(item.learning || '-')}</p><p><b>Kendala dan Solusi:</b> ${esc(item.obstacles || '-')}</p><p><b>Refleksi:</b> ${esc(item.reflection || '-')}</p><p><b>Catatan Pembimbing:</b> ${esc(item.supervisor_note || '-')}</p></div>`).join('') : '<p>Belum ada jurnal disetujui.</p>'}</section>
...
<style>
  ...
  p{margin:0 0 9pt;text-align:justify;overflow-wrap:break-word;word-wrap:break-word}.center{text-align:center}.right{text-align:right}.page-break{page-break-before:always;break-before:page}
  h1,h2,h3,h4{page-break-after:avoid;break-after:avoid}h1{font-size:18pt;text-align:center;margin:0 0 18pt}h2{font-size:15pt;text-align:center;margin:0 0 18pt}h3{font-size:12pt;margin:14pt 0 7pt}h4{font-size:12pt;margin:10pt 0 5pt}
  .cover{text-align:center}.cover-logo{width:105px;height:auto;margin:40pt 0 25pt}.cover-table{margin:0 auto;width:75%;border-collapse:collapse;table-layout:fixed}.cover-table td{padding:4pt;border:0;text-align:left;overflow-wrap:break-word}.school{margin-top:90pt;font-weight:bold;font-size:13pt}.year{margin-top:4pt}
  table{border-collapse:collapse;width:100%;margin:9pt 0;table-layout:fixed}.approval-table th,.approval-table td,.report-table th,.report-table td{border:1px solid #333;padding:5pt;vertical-align:top;overflow-wrap:break-word;word-wrap:break-word}.approval-table th{width:28%;text-align:left}.report-table{font-size:9.5pt}.report-table th{text-align:center;background:#eeeeee}
  .signature-table{table-layout:fixed}.signature-table td{width:50%;text-align:center;border:0;padding:10pt 8pt}.signature-space{height:60pt}.toc td{padding:4pt 0}.toc .leader-cell{width:100%}.leader-cell{display:flex;align-items:baseline;gap:2pt}.leader{flex:1;border-bottom:1px dotted #777;height:0}.toc-page,.page-placeholder{min-width:22pt;text-align:right}.toc td:last-child{width:100%}.figure-list .leader-cell{display:flex;align-items:baseline;gap:2pt}
  ...
  .photo-table{border-collapse:separate;border-spacing:8pt;width:100%;table-layout:fixed}.photo-cell{width:50%;vertical-align:top;text-align:center;border:1px solid #777;padding:7pt;page-break-inside:avoid;break-inside:avoid}.word-photo{width:240px;max-width:100%;max-height:205px;object-fit:contain}.word-photo-missing{height:150px;background:#eee;color:#666;text-align:center;padding-top:65px}.photo-caption{text-align:center;font-size:9pt;margin-top:5pt}
  .journal-item{border-bottom:1px solid #aaa;padding-bottom:9pt;margin-bottom:12pt;page-break-inside:avoid;break-inside:avoid}.journal-item p{font-size:10pt;margin:2pt 0}.status-warning{border:2px solid #a40000;color:#a40000;padding:8pt;text-align:center;margin-bottom:15pt}
  .note{font-size:9pt;color:#555;font-style:italic}.activity-discussion{margin-bottom:18pt;page-break-inside:avoid;break-inside:avoid}.activity-discussion-title{font-size:12pt;margin:12pt 0 6pt}.literature-link{font-size:10.5pt}.discussion-photo{width:82%;margin:12pt auto 16pt;text-align:center;page-break-inside:avoid;break-inside:avoid}.discussion-photo img{width:420px;max-width:100%;max-height:390px;object-fit:contain}.discussion-photo .photo-missing{height:180px;background:#eee;color:#666;text-align:center;padding-top:80px}.discussion-photo figcaption{font-size:9.5pt;text-align:center;margin-top:5pt}.figure-list{width:100%;border-collapse:collapse;table-layout:fixed}.figure-list td{padding:5pt;border-bottom:1px dotted #999;vertical-align:top}.figure-list td:first-child{width:80pt}.bibliography-list{margin-left:18pt}.bibliography-list li{margin-bottom:8pt;text-align:justify}

  .report-footer{font-size:8pt;color:#666;text-align:right;border-top:0;margin-top:8pt}.report-footer .page-field{font-size:9pt}
</style>
  ...
  <div class="page-break"><h2>DAFTAR ISI</h2><table class="toc">${reportTocRowsHtml()}</table><p class="note">Nomor halaman dapat diperbarui setelah dokumen dibuka dan diedit di Microsoft Word.</p></div>
  ...
  <div class="page-break"><h2>BAB III<br>PEMBAHASAN PRAKTIK KERJA LAPANGAN</h2>${reportDiscussionHtml(discussionJournals, imageSources, { totalHours })}</div>
...
<div class="report-footer">LAPORAN PRAKTIK KERJA LAPANGAN - SMK KEHUTANAN RIMBA BAHARI SUMEDANG</div></div></body></html>`;
...
async function printGroupPklReport(groupKey, { autoPrint = false } = {}) {
  ...
  try {
    ...
    const discussionJournals = reportDiscussionJournals(journals, 2);
    const photoPaths = [...new Set(discussionJournals.map((item) => (item.photo_paths || []).filter(Boolean)[0]).filter(Boolean))];
    const signedUrls = {};
    ...
    const totalHours = journals.reduce((sum,item)=>sum+(Number(item.work_hours)||0),0);
    const reportFigures = reportFigureItems(discussionJournals, { group: true });
    const reportReferences = referencesUsedByJournals(discussionJournals);
    const reviewWatermark = report.status === 'approved' ? '' : `<div class="watermark">${esc(finalReportStatusMeta(report.status)[0].toUpperCase())}</div>`;
    const logoUrl = `${location.origin}/assets/logo-sekolah.png`;
    ...
    const html = `<!doctype html><html lang="id"><head><meta charset="utf-8"><title>${esc(title)} - ${esc(internshipPlace)}</title><style>
      ...
      <section class="page prelim-page"><h2 class="chapter-title">LEMBAR PENGESAHAN</h2><div class="page-number center-page-no">i</div><p>Laporan Praktik Kerja Lapangan kelompok ini telah diperiksa dan disetujui sebagai dokumentasi pelaksanaan PKL pada lokasi yang sama.</p><table class="approval-table"><tr><th>Tempat PKL</th><td>${esc(internshipPlace)}</td></tr><tr><th>Unit Penempatan</th><td>${esc(report.placement_unit || '-')}</td></tr><tr><th>Periode PKL</th><td>${esc(period)}</td></tr><tr><th>Jumlah Anggota</th><td>${members.length} siswa</td></tr><tr><th>Status Laporan</th><td>${esc(finalReportStatusMeta(report.status)[0])}</td></tr></table><h3>Anggota Kelompok</h3>${groupMembersPrintTable(members)}<p class="right">${esc(settings.approval_location || 'Sumedang')}, ${esc(new Intl.DateTimeFormat('id-ID',{dateStyle:'long'}).format(new Date()))}</p><div class="signature-grid"><div>Pembimbing Lapangan<div class="space"></div><b>${fieldNames.length ? fieldNames.map(esc).join('<br>') : '________________________'}</b></div><div>Guru Pembimbing<div class="space"></div><b>${teacherNames.length ? teacherNames.map(esc).join('<br>') : '________________________'}</b></div><div>Perwakilan Kelompok<div class="space"></div><b>${esc(members[0]?.full_name || '________________________')}</b></div><div>Kepala Sekolah<div class="space"></div><b>${esc(principalName)}</b>${settings.principal_nip ? `<br>NIP. ${esc(settings.principal_nip)}` : ''}</div></div></section>
      <section class="page prelim-page"><h2 class="chapter-title">KATA PENGANTAR</h2><div class="page-number center-page-no">ii</div><p>${textBlock(report.preface)}</p></section>
      <section class="page prelim-page"><h2 class="chapter-title">DAFTAR ISI</h2><div class="page-number center-page-no">iii</div><table class="toc">${reportTocRowsHtml({ group: true })}</table></section>
      <section class="page prelim-page"><h2 class="chapter-title">DAFTAR GAMBAR</h2><div class="page-number center-page-no">iv</div>${reportFigureListHtml(reportFigures)}</section>
      <section class="page chapter-page"><h2 class="chapter-title">BAB I<br>PENDAHULUAN</h2><div class="page-number center-page-no">1</div><h3 class="subchapter">1.1 Latar Belakang</h3><p>${textBlock(settings.standard_background)}</p><h3 class="subchapter">1.2 Tujuan PKL</h3><p>${textBlock(settings.standard_objectives)}</p><h3 class="subchapter">1.3 Manfaat PKL</h3><p>${textBlock(settings.standard_benefits)}</p><h3 class="subchapter">1.4 Waktu dan Tempat Pelaksanaan</h3><div class="meta-box"><p><b>Tempat PKL:</b> ${esc(internshipPlace)}</p><p><b>Unit/Bagian:</b> ${esc(report.placement_unit || '-')}</p><p><b>Periode kelompok:</b> ${esc(period)}</p><p><b>Jumlah anggota:</b> ${members.length} siswa</p></div></section>
      <section class="page chapter-page"><h2 class="chapter-title">BAB II<br>PROFIL TEMPAT PRAKTIK KERJA LAPANGAN</h2><div class="page-number center-page-no">2</div><h3 class="subchapter">2.1 Identitas dan Gambaran Umum Instansi</h3><p>${textBlock(report.institution_profile)}</p><h3 class="subchapter">2.2 Struktur Organisasi / Posisi Penempatan</h3><p>${textBlock(report.organization_structure, 'Struktur organisasi tidak dicantumkan.')}</p><h3 class="subchapter">2.3 Bidang atau Bagian Penempatan Kelompok</h3><p>Kelompok melaksanakan PKL pada unit/bagian <b>${esc(report.placement_unit || '-')}</b> di ${esc(internshipPlace)}.</p></section>
      <section class="page chapter-page"><h2 class="chapter-title">BAB III<br>PEMBAHASAN PRAKTIK KERJA LAPANGAN</h2><div class="page-number center-page-no">3</div>${reportDiscussionHtml(discussionJournals, signedUrls, { group: true, membersCount: members.length, totalHours })}</section>
      
      
      <section class="page chapter-page"><h2 class="chapter-title">BAB IV<br>PENUTUP</h2><div class="page-number center-page-no">4</div><h3 class="subchapter">4.1 Kesimpulan</h3><p>${textBlock(report.conclusion)}</p>${suggestions}</section>
      <section class="page prelim-page"><h2 class="chapter-title">DAFTAR PUSTAKA</h2><div class="page-number right-page-no">5</div>${reportBibliographyHtml(reportReferences)}</section>
      <section class="page appendix-page"><h2 class="chapter-title">LAMPIRAN 1<br>DAFTAR ANGGOTA KELOMPOK</h2>${groupMembersPrintTable(members)}<h2 class="chapter-title" style="margin-top:14mm">REKAPITULASI JURNAL HARIAN KELOMPOK</h2>${groupJournalAppendixHtml(journals)}</section>
...
async function downloadGroupPklReportWord(groupKey) {
  ...
  try {
    ...
    const html = `<!doctype html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40" lang="id"><head><meta charset="utf-8"><meta name="ProgId" content="Word.Document"><meta name="Generator" content="E-Jurnal PKL"><title>${esc(title)}</title><style>
      @page Section1{size:595.3pt 841.9pt;margin:56.7pt 51pt 51pt 51pt}div.Section1{page:Section1}body{font-family:"Times New Roman",serif;font-size:12pt;line-height:1.5;color:#111;margin:0;mso-pagination:widow-orphan}p{margin:0 0 9pt;text-align:justify;overflow-wrap:break-word;word-wrap:break-word}.center{text-align:center}.right{text-align:right}.page-break{page-break-before:always;break-before:page}h1,h2,h3,h4{page-break-after:avoid;break-after:avoid}h1{font-size:18pt;text-align:center;margin:0 0 18pt}h2{font-size:15pt;text-align:center;margin:0 0 18pt}h3{font-size:12pt;margin:14pt 0 7pt}h4{font-size:12pt;margin:10pt 0 5pt}.cover{text-align:center}.cover-logo{width:105px;height:auto;margin:28pt 0 18pt}.school{margin-top:55pt;font-weight:bold;font-size:13pt}table{border-collapse:collapse;width:100%;margin:9pt 0;table-layout:fixed}.approval-table th,.approval-table td,.report-table th,.report-table td,.member-table th,.member-table td{border:1px solid #333;padding:5pt;vertical-align:top;overflow-wrap:break-word;word-wrap:break-word}.member-table th,.report-table th{text-align:center;background:#eee}.report-table{font-size:9pt}.signature-table{table-layout:fixed}.signature-table td{width:50%;text-align:center;border:0;padding:10pt 8pt}.signature-space{height:60pt}.toc td{border-bottom:1px dotted #999;padding:5pt 0}ul,ol{margin:5pt 0 9pt 24pt;padding:0}li{margin-bottom:4pt;text-align:justify}.meta-box{border:1px solid #555;padding:10pt;margin:10pt 0}.photo-table{border-collapse:separate;border-spacing:8pt;table-layout:fixed}.photo-cell{width:50%;vertical-align:top;text-align:center;border:1px solid #777;padding:7pt;page-break-inside:avoid;break-inside:avoid}.word-photo{width:240px;max-width:100%;max-height:205px;object-fit:contain}.word-photo-missing{height:150px;background:#eee;color:#666;text-align:center;padding-top:65px}.photo-caption{text-align:center;font-size:9pt;margin-top:5pt}.journal-item{border-bottom:1px solid #aaa;padding-bottom:9pt;margin-bottom:12pt;page-break-inside:avoid;break-inside:avoid}.journal-item p{font-size:10pt;margin:2pt 0}.status-warning{border:2px solid #a40000;color:#a40000;padding:8pt;text-align:center;margin-bottom:15pt}.note{font-size:9pt;color:#555;font-style:italic}.activity-discussion{margin-bottom:18pt;page-break-inside:avoid;break-inside:avoid}.activity-discussion-title{font-size:12pt;margin:12pt 0 6pt}.literature-link{font-size:10.5pt}.discussion-photo{width:82%;margin:12pt auto 16pt;text-align:center;page-break-inside:avoid;break-inside:avoid}.discussion-photo img{width:420px;max-width:100%;max-height:390px;object-fit:contain}.discussion-photo .photo-missing{height:180px;background:#eee;color:#666;text-align:center;padding-top:80px}.discussion-photo figcaption{font-size:9.5pt;text-align:center;margin-top:5pt}.figure-list{width:100%;border-collapse:collapse;table-layout:fixed}.figure-list td{padding:5pt;border-bottom:1px dotted #999;vertical-align:top}.figure-list td:first-child{width:80pt}.bibliography-list{margin-left:18pt}.bibliography-list li{margin-bottom:8pt;text-align:justify}
      ...
      <div class="page-break"><h2>KATA PENGANTAR</h2><p>${textBlock(report.preface)}</p></div>
      <div class="page-break"><h2>DAFTAR ISI</h2><table class="toc">${reportTocRowsHtml({ group: true })}</table></div>
      <div class="page-break"><h2>DAFTAR GAMBAR</h2>${reportFigureListHtml(reportFigures)}</div>
      ...
      <div class="page-break"><h2>BAB II<br>PROFIL TEMPAT PRAKTIK KERJA LAPANGAN</h2><h3>2.1 Identitas dan Gambaran Umum Instansi</h3><p>${textBlock(report.institution_profile)}</p><h3>2.2 Struktur Organisasi / Posisi Penempatan</h3><p>${textBlock(report.organization_structure, 'Struktur organisasi tidak dicantumkan.')}</p><h3>2.3 Bidang atau Bagian Penempatan Kelompok</h3><p>Kelompok melaksanakan PKL pada unit/bagian <b>${esc(report.placement_unit || '-')}</b> di ${esc(internshipPlace)}.</p></div>
      <div class="page-break"><h2>BAB III<br>PEMBAHASAN PRAKTIK KERJA LAPANGAN</h2>${reportDiscussionHtml(journals, imageSources, { group: true, membersCount: members.length, totalHours })}</div>
      
      <div class="page-break"><h2>BAB IV<br>PENUTUP</h2><h3>4.1 Kesimpulan</h3><p>${textBlock(report.conclusion)}</p><h3>4.2 Saran</h3><h4>A. Untuk Sekolah</h4><p>${textBlock(report.suggestions_school)}</p><h4>B. Untuk Tempat PKL</h4><p>${textBlock(report.suggestions_workplace)}</p><h4>C. Untuk Siswa</h4><p>${textBlock(report.suggestions_students)}</p></div>
      <div class="page-break"><h2>DAFTAR PUSTAKA</h2>${reportBibliographyHtml(reportReferences)}</div>
      <div class="page-break"><h2>LAMPIRAN 1<br>DAFTAR ANGGOTA DAN REKAP JURNAL</h2><table class="member-table"><thead><tr><th>No</th><th>Nama</th><th>NISN</th><th>Kelas</th></tr></thead><tbody>${membersRows}</tbody></table>${groupJournalAppendixHtml(journals)}</div>
