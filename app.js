...
window.addEventListener('load',function(){
  ...
  Promise.all(waits).then(()=>{
    const pageBoxHeight=297*(96/25.4);
    const pageNumberFor=(el)=>{
      if(!el) return '-';
      const top=el.getBoundingClientRect().top+window.scrollY;
      return String(Math.max(1,Math.floor((top+1)/(pageBoxHeight))+1));
    };
    ...
    const materialHeads=[...document.querySelectorAll('.material-discussion-title')];
    const subHeads=[...document.querySelectorAll('.activity-discussion-title')];
    materialGroups.forEach((mg,mi)=>{
      const mc=document.querySelector('.toc-page[data-target="toc-target-material-'+(mi+1)+'"]');
      if(mc && materialHeads[mi]) mc.textContent=pageNumberFor(materialHeads[mi]);
      mg.subgroups.forEach((sg,si)=>{
        const sc=document.querySelector('.toc-page[data-target="toc-target-sub-'+(mi+1)+'-'+(si+1)+'"]');
        const gi=materialGroups.slice(0,mi).reduce((n,x)=>n+x.subgroups.length,0)+si;
        if(sc && subHeads[gi]) sc.textContent=pageNumberFor(subHeads[gi]);
      });
    });
